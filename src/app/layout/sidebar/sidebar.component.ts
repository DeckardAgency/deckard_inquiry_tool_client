...
export class SidebarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.supportModalService.isOpen$.pipe(
      takeUntilDestroyed(this.destroyRef)
    ).subscribe(isOpen => {
  ...
  updateUserDisplay(): void {
    if (this.currentUser) {
      ...
      if (this.currentUser.roles && this.currentUser.roles.length > 0) {
        ...
        // Convert ROLE_USER to User, ROLE_CLIENT_ADMIN to Client Admin, etc.
        this.userRole = role.replace('ROLE_', '').split('_')
  ...
  toggleOrdersMenu(): void {
    this.isOrdersExpanded.update(value => !value);
  }
  ...
  /**
   * Whether to show the Pending Approvals link in the sidebar
   * Visible only when the client has order or inquiry approval enabled
   */
  toggleViewMode(): void {
    this.viewMode.update(mode => mode === 'admin' ? 'client' : 'admin');
  }

  get showPendingApprovals(): boolean {
    return !!(
      this.currentUser?.client?.requiresOrderApproval ||
      this.currentUser?.client?.requiresInquiryApproval
    );
  }
