...
export class DraftOrdersComponent implements OnInit {
  ...
  constructor(
    ...
    this.logger = this.loggerService.createLogger('DraftOrdersComponent');
  }
  ...
  onView(item: DraftListItem): void {
    if (item.type === 'Inquiry') {
      ...
      this.router.navigate(['/my-orders/active/order', item.id, 'view']);
    }
  }
  ...
  private editDraftOrder(item: DraftListItem): void {
        ...
        .subscribe((products: (Product | null)[]) => {
          ...
          products.forEach((product: Product | null, index: number) => {
            if (product) {
              ...
              // Use unitPrice from order item if available, otherwise use product price
    ...
    if (item.originalData && 'items' in item.originalData) {
      ...
      this.orderService.getOrder(item.id)
        ...
        .subscribe((order) => {
          if (order) {
            loadOrderAndProducts(order as OrderResponse); // Order type from API
          }
        });
    }
  }
      ...
      if (machine.products && machine.products.length > 0) {
        machine.products.forEach((product, index: number) => {
          ...
          const files = (product.mediaItems?.map((mediaItem) => ({
            ...
            progress: 100,
            mediaItem: mediaItem,
            file: null // Actual file object not available when loading from server
          })) || []);
